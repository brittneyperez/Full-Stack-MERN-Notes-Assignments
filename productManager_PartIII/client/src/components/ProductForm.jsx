...
const ProductForm = ({ formProducts, setFormProducts }) => {
    ...
    const submitHandler = (e) => {
        ...
        axios.post('http://localhost:8000/api/products', {
            productName, productPrice, productDescription // Shortcut for: productName: productName, productPrice: productPrice, productDescription: productDescription
        })
            .then(res => {
                console.log(res);
                console.log(res.data)
                setFormProducts([...formProducts, res.data]); // from props
    ...
    return (
        <div className="w-full max-w-lg mx-auto">
            <form onSubmit={ submitHandler } className="bg-white shadow-md rounded px-5 pt-6 pb-8 mb-4">
                
                <h2 className="mt-3 font-serif text-3xl font-semibold text-stone-700">Poduct Management Form</h2>
                {/* Product Name */}
                ...
                <div className="md:flex md:items-center my-6">
                    ...
                    <div className="md:w-2/3">
                        <input onChange={(e) => setProductDescription(e.target.value)} value={ productDescription } type="text"
                            className="bg-stone-100 appearance-none border-2 border-stone-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-teal-500"
                        />
                    </div>
                </div>
                ...
                <div className="flex justify-center">
                    <input type="submit" value="Create Product" className="shadow bg-teal-500 hover:bg-teal-300 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" />
                </div>
            </form>
        </div>
    )
